...
const Int_t nr_of_x=3;
const Int_t nr_of_phi=1;
const Int_t nr_of_th=5;
...
filename[0]="theta-calib-run110.dat";   
filename[1]="theta-calib-run113.dat";   
filename[2]="theta-calib-run114.dat";      
TString pathname="/home/lmdonaldson/K600/Analysis/PR373/";
...
TPaveLabel *title = new TPaveLabel(0.1,0.96,0.9,0.99,"#theta_{fp} to #theta_{scat} Conversion");
...
for(Int_t k=0;k<nr_of_x;k++) {
  for(Int_t n=0;n<nr_of_phi;n++){
    gr_angle[k][n]= new TGraph(5,ThFP[k][n],ThSC[k][n]);
...
TGraph *gr_TH[2];
gr_TH[0]= new TGraph(3,Xtmp,slopetmp);
gr_TH[1]= new TGraph(3,Xtmp,offsettmp);
